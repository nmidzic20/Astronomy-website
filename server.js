...
server.get("/multimedija", (zahtjev, odg) => {
  ...
  for (obj of urls) {
    if (
      obj.url.endsWith(".png") ||
      obj.url.endsWith(".jpg") ||
      obj.url.endsWith(".jpeg")
    ) {
      images.push(obj.url);
    }
  ...
  tekst +=
    "<article id='clanak_slike'><h2>Slike</h2><div id='grid'><div class='grid-kontejner'>";
  for (let i = 0; i < images.length; i++) {
    tekst += "<figure><img class='ispis' src='" + images[i] + "'/></figure>";
  }
  tekst +=
    "</div></div></article><div class='grid-kontejner2'><article id='clanak_video'><h2>Videi</h2><div class='grid-kontejner3'>";
  for (let i = 0; i < videos.length; i++)
    tekst += "<video controls src='" + videos[i] + "'></video>";
  tekst +=
    "</div></article><article id='clanak_audio'><h2>Audio</h2><div id='grid-kontejner4'>";
...
function stvoriTablicuCSV() {
  ...
  tablica +=
    '<table id="popis_korisnika"><caption id="naslov-tablice">Podaci o novim članovima astronomskog kluba</caption>';
