...
if (!postojiKolacic && !lokacija.includes("prijava")) {
    ...
    if (lokacija.includes("kreativna") ||
    ...
    document.addEventListener("DOMContentLoaded", function () {
        if (document.URL.indexOf("?") > -1) {
            htmlDokument = document.URL
                .substring(document.URL.lastIndexOf("/") + 1, document.URL.indexOf("?"));
        }
        else {
            htmlDokument = document.URL
                .substring(document.URL.lastIndexOf("/") + 1);
        }
        console.log(htmlDokument);

        for (i = 0; i < kolacic.length; i++) 
        {
            if (kolacic[i].includes("boja") || kolacic[i].includes("fontsize") 
            || kolacic[i].includes("logoVerzija") || kolacic[i].includes("bojatekst")
            || kolacic[i].includes("visina"))
            {
                ...
                if (htmlDokument != "prijava.html" && htmlDokument != "dizajn.html"
                ...
                if (k == "v1")
                {
                    if (lokacija.includes("kreativna") ||
                        ...
                        lokacija.includes("multimedija") ||
                        lokacija.includes("popis"))
                        document.getElementById("verzijaLoga").src = "../materijali/logo.jpg";
                    else if (htmlDokument != "prijava.html" && htmlDokument != "dizajn.html"
                    && htmlDokument != "pregled.html")
                        document.getElementById("verzijaLoga").src = "./materijali/logo.jpg";
                }
                else if (k == "v2")
                {
                    if (lokacija.includes("kreativna") ||
                        ...
                        lokacija.includes("multimedija") ||
                        lokacija.includes("popis"))
                        document.getElementById("verzijaLoga").src = "../materijali/orion.jpg";
                    else if (htmlDokument != "prijava.html" && htmlDokument != "dizajn.html"
                    && htmlDokument != "pregled.html")
                        document.getElementById("verzijaLoga").src = "./materijali/orion.jpg";
                }
                ...
                if (k == "bijela")
                    document.body.style.color = "rgb(233, 225, 225)";
                else if (k == "siva")
                    document.body.style.color = "rgb(125, 123, 122)";
...
function otvoriPoveznicu() {
    ...
    if (navigacija[0] != undefined && navigacija[0] != null && htmlDokument != "kreativna.html") {
...
function dodajSlusac(i) {
    poveznica[i].addEventListener("click", function (event) {
        ...
        skocni = "<dialog open id='dijaloski'><p id='tekstProzora'>Sigurno želite napustiti stranicu?</p><button id='ok'>OK</button><button id='cancel'>Cancel</button></dialog>";
    ...
    if (inputURL.includes("?")) 
    {
        ...
        for (let obj of parametriJSON)
        {
            Object.keys(obj).forEach( function(kljuc) {
                let x = kljuc;
                let y = obj[kljuc];
                tekst += "GET parameter " + x + " i vrijednost " + y + " nije dozvoljen na ovoj stranici\n";
            });
        }
...
function provjeraObrasca() {
    ...
    obrazac.addEventListener("submit", function (event) {
        ...
        for (l of labele)
            if (l.innerHTML.indexOf("*") > -1) {
                ...
                if (l.id != null && (l.id == "checkboxPolje" || l.id == "radiobutton"))
                    ...
                    document.getElementById(l.htmlFor).style.border = "";
            }
        ...
        if (cjeline[0].style.display != "none")
            if (!radioOdabir) greska.push(document.getElementById("radiobutton").innerHTML);

        if (cjeline[2].style.display != "none")
            if (brChecked < 2) greska.push(document.getElementById("checkboxPolje").innerHTML);
...
function animPovecajSmanji() {
    ...
    anim1.addEventListener("click", function (event) {
        ...
        disableClick = true;
        setTimeout(() => { disableClick = false; }, 1000); //1 sek do ponovnog omogucavanja utjecaja klika
...
function animRotiraj() {
    ...
    slikaRot = [];
    for (s of slikeRot) { slikaRot.push(s);}
    trenutnaRot = 0;
...
function animPoZelji() {
    ...
    var promatrac = new IntersectionObserver(function(entries) {
...
function kreirajKolacicePrijava() 
{
    ...
    document.cookie = "korime=" + korime + ";expires=" + istice + ";SameSite=Lax;path=/";
    document.cookie = "lozinka=" + lozinka + ";expires=" + istice + ";SameSite=Lax;path=/";
    document.cookie = "vrijeme=" + vrijemeTrajanja / 1000 + ";expires=" + istice + ";SameSite=Lax;path=/";
    document.cookie = "datumIsteka=" + istice + ";expires=" + istice + ";SameSite=Lax;path=/";
}
...
function ispisKolacica() 
{
    ...
    mjestoIspisa.innerHTML = "<p>Korisničko ime: " + document.cookie.split(";")[0].split("=")[1] + "</p>" +
        "<p>Datum i vrijeme do kad vrijedi kolačić: " + document.cookie.split(";")[3].split("=")[1] + "</p>";
}
...
function nazivLinkaPrijavaOdjava() 
{
    ...
    if (prijavaOdjava) {
        if (postojiKolacic)
            prijavaOdjava.innerHTML = "Odjava";
        else
...
function otvoriProzorDizajn() 
{
    ...
    gumbDizajn.addEventListener("click", function () {
        ...
        if (prozor == null)
        {
            ...
            lokacija.includes("popis"))
            {
                prozor = window.open("../ostalo/dizajn.html", "Korisnički dizajn", "width=400, height=400");     
            }
            else
                prozor = window.open("./ostalo/dizajn.html", "Korisnički dizajn", "width=400, height=400");     
...
function promijeniDizajn()
{
    ...
    dizajnGumb.addEventListener("click", function(event) {
        ...
        var logoVerzija = document.getElementById("logoVerzija").selectedOptions[0].value;
        var bojatekst = document.getElementById("bojatekst").selectedOptions[0].value;
        console.log("hi");
        let istek = document.cookie.split(";")[3].split("=")[1];
        document.cookie = "boja=" + boja + ";expires=" + istek + ";SameSite=Lax;path=/";
        document.cookie = "fontsize=" + fontsize + ";expires=" + istek + ";SameSite=Lax;path=/";
        document.cookie = "visina=" + visina + ";expires=" + istek + ";SameSite=Lax;path=/";
        document.cookie = "logoVerzija=" + logoVerzija + ";expires=" + istek + ";SameSite=Lax;path=/";
        document.cookie = "bojatekst=" + bojatekst + ";expires=" + istek + ";SameSite=Lax;path=/";
        prozor = null;
        window.close();
    });
}
...
function statistika()
{
    ...
    grafGumb.addEventListener("click", function(event)
    {
        ...
        objektGrafa = document.getElementById("objektGrafa").selectedOptions[0].value;
...
function stvoriGraf()
{
    ...
    kontekst.fillRect(objekt1.x1,objekt1.y1,objekt1.sirinaObjekta,objekt1.visinaObjekta);
    kontekst.fillStyle = objekt2.bojaObjekta;
    kontekst.fillRect(objekt2.x1,objekt2.y1,objekt2.sirinaObjekta,objekt2.visinaObjekta);
